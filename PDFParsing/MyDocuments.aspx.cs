...
namespace PDFParsing
{
    public partial class MyDocuments : System.Web.UI.Page
    {
        ...
        {
            int Srno = 1;
            var DATA = from a in dbcontext.DOCUMENTs.AsEnumerable().Select((r, i) => new { Row = r, Index = i }).Where(x => x.Row.USER_ID == Convert.ToInt32(User.Identity.Name)).ToList()
                       select new
                       {
                           INDEX = Srno++,
            ...
            //IEnumerable<DOCUMENT> Documents = dbcontext.DOCUMENTs.Where(x => x.USER_ID == Convert.ToInt32(User.Identity.Name)).OrderByDescending;
                ...
                if (parse != null)
                {
                    ...
                    var data = from a in dbcontext.PARSER_SHEETs.Where(x => x.PARSER_ID == PARSER_ID)
                               select new
                               {
                                   ...
                                   CHQ_NO = a.CHQ_NO,
                                   VALUE_DATE = a.DATE.Value.Day.ToString() + "/" + a.DATE.Value.Month.ToString() + "/" + a.DATE.Value.Year.ToString(),
